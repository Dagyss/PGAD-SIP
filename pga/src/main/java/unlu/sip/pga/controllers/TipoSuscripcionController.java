...
public class TipoSuscripcionController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<TipoSuscripcionDTO> getTipoSuscripcion(@PathVariable Integer id) {
        return service.getTipoSuscripcion(id).map(mapper::toDto).map(ResponseEntity::ok)
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteTipoSuscripcion(@PathVariable Integer id) {
        if (service.deleteTipoSuscripcion(id)) {
